...
class VerifyEmailAddress(object):
    ...
    def __init__(self):
        ...
        self.emial_verification_logger = logging.getLogger(__name__) #Logging
    ...
    def checkValidity(self, email_address):
        ...
        try: 
            ...
            if code == 250:
                self.email_verification_logger.info('The email address : %r \
            ...
            self.email_verification_logger.error('Got : %r while verifying : %r.'\
...
class FileInfo(object):
    ...
    def __init__(self):
        self.file_info_logger = logging.getLogger(__name__) #Logging
    ...
    def getExistanceStatus(self, file_path):
        ...
        try:
            ...
            if (existing_file == True):
                self.file_info_logger.info('The file : %r exists on the disk. \
                                  So we can proceed further.' \
                                  %file_path)
                return 'YES'
            else:
                self.file_info_logger.warn('The file : %r does not exist on \
                                  the disk. So email sending was \
                                  failed.' %file_path))
                return 'NO'
        except Exception as existance_error:
            self.file_info_logger.error('Got : %r while checking existance of : %r.'\
                              %(repr(existance_error), file_path))
            return {'Existance Error : %r.' %repr(existance_error)}
    ...
    def getType(self, file_path):
        ...
        try:
            ...
            if extension in accepted_types:
                self.file_info_logger.info('The file : %r is of type : %r.' \
                                  %(file_path, extension))
                return 'Accepted'
            else:
                self.file_info_logger.warn('The file : %r cannot be send by email \
                                  as it is of unsupported type.' %file_path)
                return 'Rejected'
        except Exception as type_error:
            self.file_info_logger.error('Got : %r while checking extension of : %r.'\
                               %(repr(type_error), file_path))
    ...
    def getSize(self, file_path):
        ...
        try:
           ...
           if (file_size <= 25000000):
               self.file_info_logger.info('The file : %r is of size : %r bytes , \
                                 less than 25MB. Hence fine for sending \
                                 the email.' %(file_path, file_size)) 
               return 'Perfect'
           else:
               self.file_info_logger.warn('The file is too large to attach.')
               return 'Imperfect'
        except Exception as file_size_error:
            self.file_info_logger.error('During checking the size of : %r got : %r.'\
...
class Compressor(object):
    ...
    def __init__(self):
        ...
        self.compression_logger = logging.getLogger(__name__) #Logging
    
    def compressFile(self, FilePath):
        
        try:
            self.compression_logger.info('The file : %r is larger than 25MB. \
            ...
            self.compression_logger.info('After compression, the file size \
                                   becomes : %r.' %float(getsize(zip_file)))
            zip_file.close()
            return FilePath
        except Exception as compressing_error:
            self.compression_logger.error('Got : %r while compressing : %r.'\
                                 %(repr(compressing_error), FilePath))
            return {'Error : %r.' %repr(compressing_error)}



class Attchment(FileInfo, Compressor):
    '''
    ...
    def __init__(self):
        ...
        self.attachment_logger = logging.getLogger(__name__) #Logging 
    ...
    def prepare(self, file_path):
        ...
        if (self.getExistanceStatus(file_path)=='YES' \
            and self.getType(file_path)=='Accepted' \
            and self.getSize(file_path)=='Perfect'):
            self.attachment_logger.info('The file : %r is fine \
                                   for attaching.' %(file_path))
            file_name,extension = splitext(file_path)
            if (extension == '.json'):
                ...
                encoders.encode_base64(attachment) 
                fp.close()
        elif (self.getExistanceStatus(file_path)=='YES' \
              and self.getType(file_path)=='Accepted'\
              and self.getSize(file_path)=='Imperfect'):
            new_path = compressFile(file_path)
            if (self.getSize(new_path) == 'Perfect'):
                self.mail_logger.info('After compressing,the file : %r and \
                                  it is of size : %r bytes, less than 25MB.'\
                                  %(new_path))
                zipped_data = open(new_path, 'rb').read()
                attachment = MIMEApplication(zipped_data, extension)
            else:
                self.mail_logger.warn('After compressing the file : %r , \
                                  still it cannot be attached to the \
                                  mail because it is too large.'\
                                  %new_path)  
                attachment = None
        else:
            self.mail_logger.warn('The attachment does not exist in the proper location/not \
...
class EmailBody(FileInfo):
    ...
    def __init__(self):
        self.email_body_logger = logging.getLogger(__name__)
        ...
        if (self.getExistanceStatus(body_text) == 'YES'):              
            try:
                with open(body_text) as fp:
                    email_body = fp.read()
                    fp.close()
                self.email_body_logger.info('The program is reading \
                mail body from : %r.' %body_text) 
                return email_body
            except Exception as body_error:
                self.email_body_logger.error('Got : %r while reading \
                email body from : %r.' %(repr(body_error), body_text))
                return {'Error : %r.' %repr(body_text)}
        else:
            self.email_body_logger.warn('The file : %r supplied for the \
            body of the email could not be found in the proper path.'\
            %body_text)
...
class Mailer(VerifyEmailAddress, FileInfo, EmailBody, Attchment):
    
    def __init__(self):
        
        self.mail_logger = logging.getLogger(__name__) #Logging
    ...
    def send(self, sender, app_password, receiver, cc=[], bcc=[], reply_to=[], subject=None, body_text=None, attachments=[]):
        ...
        self.mail_logger.info('Got all the components for sending an email at : %r.'\
        ...
        if (self.checkValidity(sender) == 'invalid'):
             self.logger.warn('The sender email address : %r is incorrect. \
                               So email sending has been failed.' %sender)
             sys.exit()
        else:
             msg['From'] = sender
        ...
        if (type(receiver_list) == list and len(receiver_list) != 0):
            ...
            self.mail_logger.info('According to receivers list provided by the user, \
                              valid email addresses among them are : %r.' \
                              %valid_receivers)
            self.mail_logger.warn('According to receivers list provided by the user, \
                              the invalid email addresses among them are : %r.'\
                              %invalid_receivers)
            try:
                msg['To'] = ','.join(valid_receivers)
            except Exception as receiver_error:
                self.mail_logger.warn('The email addresses : %r cannot \
            ...
            logger.mail_error('No receipients has been added for this email')
        ...
        if (type(cc_list) == list and len(cc_list) != 0):
            ...
            self.mail_logger.info('According to CC list provided by the user, \
                              valid email addresses among them are : %r.' \
                              %valid_cc_list)
            self.mail_logger.warn('According to CC list provided by the user, \
                              the invalid email addresses among them are : %r.'\
                              %invalid_cc_list)
            try:
            ...
            self.mail_logger.info('No Carbon Copy receipient has been added for this email')
        ...
        if (type(bcc_list) == list and len(bcc_list) != 0):
            ...
            self.mail_logger.info('According to BCC list provided by the user, \
                              valid email addresses among them are : %r.'\
                              %valid_bcc_list)
            self.mail_logger.warn('According to BCC list provided by the user, \
                              the invalid email addresses among them are : %r.'\
                              %invalid_bcc_list)
            try:
                msg['Bcc'] = ','.join(valid_bcc_list)
            except Exception as bcc_error:
                self.mail_logger.warn('The email addresses : %r cannont be added \
                                  as bcc due to : %r.'\
                                  %(valid_bcc_list, bcc_error))
                raise Exception('Error : The email addresses : %r cannont be added \
                                 in bcc field.' %valid_bcc_list)
        else:
            self.mail_logger.info('No Blind Carbon Copy receipient has been added for this email')    
        ...
        if (type(reply_to_list) == list and len(reply_to_list) != 0):
            ...
            self.mail_logger.info('According to Reply To list provided by the user, \
                              valid email addresses among them are : %r.' \
                              %valid_reply_list)
            self.mail_logger.warn('According to Reply To list provided by the user, \
                              from it invalid email id list among is : %r.'\
                              %invalid_reply_list)
            try:
                msg['Reply-To'] = ','.join(valid_reply_list)
            except Exception as reply_to_error:
                self.mail_logger.warn('The email addresses : %r cannont be added in \
                                  reply_to due to : %r.'\
            ...
            self.mail_logger.info('No email addres has been added for reply to field in this email')
            ...
            for file_path in attachments:
                ...
                try:
                    ...
                    if (attachment != None):  
                        ...
                        self.mail_logger.info('From the path : %r, file found is : %r  and \
                                          we are attaching this file with the mail.'\
                                          %(file_path, filename))
                    else:
                        self.mail_logger.warn('File : %r hasn\'t been attached as it doesn\'t \
                    ...
                    self.mail_logger.warn('The file : %r cannot be attached along with \
                                      this email due to : %r.' \
                                      %(filename, attachment_error))
                    pass
        else:
            self.mail_logger.error('Attachment paths are not provided in a list. \
                          So cannot attach these files in this email.')
            raise Exception('Attachment paths should be provided in a list.')


        self.mail_logger.info('The SMTP server is getting started at : %r.'\
                          %dt.now().strftime('%Y-%m-%d-%H:%M:%S'))
        ...
        server.starttls()
        self.mail_logger.info('The server is logging in to the sender\
                               email address at : %r.'\
                               %dt.now().strftime('%Y-%m-%d-%H:%M:%S'))
        ...
        self.mail_logger.info('The mail has been sent at : %r.'\
                        %dt.now().strftime('%Y-%m-%d-%H:%M:%S'))
        # Closing the connection with SMTP server.
        server.quit()
        self.mail_logger.info('The SMTP server is getting closed at : %r.' \
                          %dt.now().strftime('%Y-%m-%d-%H:%M:%S'))
